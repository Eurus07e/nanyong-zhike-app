...
备忘录正文会持久保存在运行南雍知课的同一个 SQLite 数据库中，按统一身份认证学号隔离，不发送给 Memos 或其他第三方。删除备忘录后，对应记录会从数据库删除。五育与第二课堂数据按需从学校系统查询，只在当前服务和浏览器内存中短期缓存，不写入本站数据库。使用本地桌面版时，数据只保存在下方本机数据目录；使用他人维护的共享部署时，服务维护者能够接触服务器上的数据库，因此只应使用可信部署。
